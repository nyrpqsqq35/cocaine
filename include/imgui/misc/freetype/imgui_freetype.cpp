...
namespace
{
    ...
    bool FreeTypeFont::InitFont(FT_Library ft_library, const ImFontConfig& cfg, unsigned int extra_font_builder_flags)
    {
        ...
        if (UserFlags & ImGuiFreeTypeBuilderFlags_Stroke) {
            error = FT_Stroker_New(ft_library, &Stroker);
            if (error != 0)
                return false;
            FT_Stroker_Set(Stroker, 1 * 64, FT_STROKER_LINECAP_BUTT, FT_STROKER_LINEJOIN_ROUND, 0);
        }
    ...
    const FT_Bitmap* FreeTypeFont::RenderGlyphAndGetInfo(GlyphInfo* out_glyph_info)
    {
        if (Stroker) {
            ...
            error = FT_Glyph_To_Bitmap(&glyph, RenderMode, nullptr, true);
            FT_BitmapGlyph bitmapGlyph = reinterpret_cast<FT_BitmapGlyph>(glyph);
            FT_Bitmap* bitmap = &bitmapGlyph->bitmap;

            out_glyph_info->Width = (int)bitmap->width;
            out_glyph_info->Height = (int)bitmap->rows;
            out_glyph_info->OffsetX = bitmapGlyph->left;
            out_glyph_info->OffsetY = -bitmapGlyph->top;
            out_glyph_info->AdvanceX = (float)FT_CEIL(Face->glyph->advance.x);
            out_glyph_info->IsColored = (bitmap->pixel_mode == FT_PIXEL_MODE_BGRA);
