...
class GLCore {
  public:
    static void checkError(const char* filename, const uint32_t line,
                           const char* expression);
    static GLuint compileShaderFromCode(GLenum shader_type, const char* src);
    static GLuint compileShaderFromFile(GLenum shader_type,
                                        const char* filepath);
  ...
  #define GLCHECK(expr) \
    expr;               \
    GLCore::checkError(__FILE__, __LINE__, #expr);
#else
